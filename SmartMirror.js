...
var rollover = false; // Is the mouse hovering over 
...
function preload() {
  ...
  var newsUrl = 'https://newsapi.org/v2/top-headlines?sources=techcrunch&apiKey=ebb9aecdae8c4a7b8b217b7bdb170aad';
...
function draw() {
  ...
  if(state == true) {
    ...
    for (i = 1; i < weatherList.length; i++)
    {
      ...
      if (showDay == 1)
      {
        text("MON", x3 + xx  - 30, y3 + yy- 415, w3, h3);
      }
      if (showDay == 2)
      {
        text("TUE", x3 + xx  - 30, y3 + yy- 415, w3, h3);
      }
      if (showDay == 3)
      {
        text("WED", x3 + xx  - 30, y3 + yy- 415, w3, h3);
      }
      if (showDay == 4)
      {
        text("THU", x3 + xx  - 30, y3 + yy- 415, w3, h3);
      }
          if (showDay == 5)
      {
        text("FRI", x3 + xx  - 30, y3 + yy- 415, w3, h3);
      }
          if (showDay == 6)
      {
        text("SAT", x3 + xx  - 30, y3 + yy- 415, w3, h3);
      }
          if (showDay == 0)
      {
        text("SUN", x3 + xx  - 30, y3 + yy- 415, w3, h3);
      }
      data = weatherList[i];
      text('L:', x3 + 70, y3 + yy- 415, w3, h3);
      text(int(data.temp.min), x3 + 90, y3 + yy- 415, w3, h3);
      
      text('H:', x3 + 120, y3 + yy- 415, w3, h3); 
      text(int(data.temp.max), x3 + 140, y3 + yy- 415, w3, h3); 
      ...
      img = createImg(url);
      img.hide(); 
      image(img, x3 + 170, y3 + yy- 432, 50, 50); 
      print(url);
    ...
    var lineStart = 0;
    var lineEnd = 0;
    if (articleOneShown == 0 && mouseX > 775 + 18 && mouseY < 500)
    {
      for (i = 0; i < description.length; i++)
      {
        if (i % 60 == 1)
        {
          lineEnd = i-1;
          textAlign(CENTER);
          text(description.slice(lineStart, lineEnd), width*0.5, yy2 -50);
          yy2  += 20;
          lineStart = lineEnd;
        }
      }
      textAlign(CENTER);
      text(description.slice(lineStart, description.length), width*0.5, yy2 -50);
    }
...
function powerButton() {
  ...
  if(state == true) {
    offButton = createImg('PowerON.png');
  ...
  if(state == false) {
    ...
    onButton = createImg('PowerOFF.png');    
